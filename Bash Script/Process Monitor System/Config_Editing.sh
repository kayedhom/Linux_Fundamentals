...
Change_Config() {
    ...
    local new_CPU_ALERT_THRESHOLD=$CPU_ALERT_THRESHOLD
    local new_MEMORY_ALERT_THRESHOLD=$MEMORY_ALERT_THRESHOLD
    ...
    case $option in
        ...
        E)
            ...
            ;;
        F)
            read -p "Enter new Cpu Alert usage threshold: " new_CPU_ALERT_THRESHOLD
            ;;
        G)
            read -p "Enter new Memory Alert usage threshold: " new_MEMORY_ALERT_THRESHOLD
            ;;
        *)
            echo "Invalid option."
            return
    ...
    case $option in
        ...
        log_configuration_change "Cpu Alert usage threshold changed to $new_CPU_ALERT_THRESHOLD%"
        update_configuration "$config_file" "$CPU_ALERT_THRESHOLD" "$new_CPU_ALERT_THRESHOLD"
        ;;
        
        G) 
        MEMORY_ALERT_THRESHOLD=$new_MEMORY_ALERT_THRESHOLD
        log_configuration_change "Memory Alert usage threshold changed to $new_MEMORY_ALERT_THRESHOLD%"
        update_configuration "$config_file" "$MEMORY_ALERT_THRESHOLD" "$new_MEMORY_ALERT_THRESHOLD"
        ;;
...
Config_Print(){
        echo  "Refresh Rate Display System statistics $REFRESH_TIME sec."
        echo  "Minimum Cpu Usage Filtration--> $CPU_MIN% ."
        echo  "Maximum Cpu Usage Filtration--> $CPU_MAX% ."
        echo  "Minimum Memory Usage Filtration--> $MEM_MIN% ."
        echo  "Maximum Memory Usage Filtration--> $MEM_MAX% ."
        echo -e "Cpu Alert usage threshold--> $CPU_ALERT_THRESHOLD% ."
        echo -e "Memory Alert usage threshold--> $MEMORY_ALERT_THRESHOLD% .\n\n"
...
Config_Set(){
	...
	Config_Print
	 ...
	 echo "6. Cpu Alert usage threshold"
	 echo "7. Memory Alert usage threshold"
	 echo "8. Back to Main"
